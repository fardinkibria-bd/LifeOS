...
export function AnimatedDock({ collapsed, onToggleCollapse }: { collapsed: boolean; onToggleCollapse: () => void }) {
  ...
  return (
    ...
    >
      <div className="lifeos-sidebar-brand flex h-[68px] items-center gap-3 px-4 shrink-0">
        <div className="lifeos-sidebar-mark flex h-9 w-9 items-center justify-center shrink-0">
          <LifeOSLogo className="h-7 w-7" />
        </div>
        {!collapsed && (
          <span className="lifeos-sidebar-wordmark">LifeOS</span>
        )}
      </div>
      ...
      <div className="lifeos-sidebar-actions flex flex-col gap-2 p-3">
        ...
        >
          ...
          {!collapsed && <kbd className="ml-auto text-caption text-text-muted">Cmd K</kbd>}
        </button>
      </div>

      <nav className="lifeos-sidebar-nav flex-1 overflow-y-auto scrollbar-thin py-3" aria-label="LifeOS navigation">
        {navGroups.map(group => (
            ...
            {group.items.map(item => {
              ...
              return (
                <button
                  ...
                  style={{ width: 'calc(100% - 24px)', transformOrigin: 'center center' }}
      ...
      <div className="lifeos-sidebar-profile p-3 flex items-center gap-2">
        ...
        >
          ...
          {!collapsed && (
            <span className="lifeos-sidebar-user truncate">{profile?.display_name || 'User'}</span>
          )}
        </button>
        {!collapsed && (
          <button
            onClick={onToggleCollapse}
            className="lifeos-sidebar-collapse flex h-8 w-8 items-center justify-center shrink-0"
          >
      ...
      {collapsed && (
        <button
          onClick={onToggleCollapse}
          className="lifeos-sidebar-collapse flex h-9 w-full items-center justify-center"
        >
