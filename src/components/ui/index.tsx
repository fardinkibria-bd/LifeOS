import { ReactNode, CSSProperties, useRef, useCallback } from "react";
...
export function GlassCard({
  ...
  const variants: Record<string, string> = {
    ...
    focus:
      "glass-strong glass-highlight shadow-lg border-accent/30 glow-sm-primary",
  };
  return (
    <div
      onClick={onClick}
...
export function ProgressBar({
  ...
  return (
    <div
      className={cn(
        "h-1.5 w-full rounded-full bg-surface-active overflow-hidden",
        className,
      )}
    >
      <div
        className={cn(
          "h-full rounded-full transition-all duration-400 ease-out-quart",
          colors[variant],
        )}
        style={{ width: `${pct}%` }}
      />
